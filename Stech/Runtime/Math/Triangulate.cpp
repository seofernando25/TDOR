...
bool Triangulate::Snip(const std::vector<Vector2> &contour, int u, int v, int w, int n, std::vector<int> &V) {
    int p;
    ...
    if ( std::numeric_limits<float>::epsilon() > (((Bx-Ax)*(Cy-Ay)) - ((By-Ay)*(Cx-Ax))) ) return false;
